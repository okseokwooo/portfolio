...
스마트모바일 프로그래밍 프로젝트 주제는 "갖다드림"으로 선택했다. 주제를 선택한 이유는 교내에서 누군가 대신 해주었으면 하는 심부름이 필요한 상황을 종종 겪게되는데 이에대한 편의를 제공하기 위해 생각해냈다. 상명대학교 학생들을 위한 심부름 서비스 Application이고, 캠퍼스 내 시설 또는 캠퍼스 근방 학생들에게 심부름 서비스를 제공한다. 심부름 시키는 사람은 수행하는 사람에게 일정한 대가를 지급하여 어플이 순환한다. 따로 서버를 구현하여 채팅방 기능 및 로그인 회원가입을 수행했다.
...
캡스톤 디자인 주제로 딥러닝 소리 분석에 기반한 이어폰을 개발했다. 노이즈 캔슬링 이어폰은 주변 소리를 통해 얻을 수 있는 정보를 차단하고 있어 문제가 야기될 가능성이 높다. 노이즈캔슬링 이어폰을 착용한다면 경적소리, 사이렌 소리 등 각각 위험한 상황에 노출될 가능성이 크다. 이러한 상황을 방지하기 위해 CNN 모델을 통한 딥러닝 기술을 이용한다. 스마트폰에 내장된 마이크를 통해 위에서 말한 특정 위험 소리를 감지한다. 파이썬으로 전사 데이터 특징을 MFCC로 추출하여 이 특징을 CNN에 학습시킨다. 학습된 데이터를 통해 마이크로 감지한 소리를 판별하여 위험신호일 경우에 사용자에게 알려준다. 이때 사용한 전사 데이터는 UrbanSound8K 8732개의 도시 사운드를 4초 이하로 편집 되어있는 자료를 사용하였다. 이 사운드 들은 경적소리, 개 짖는 소리, 싸이렌 소리 등 총 10개의 클래스로 구분되어 있이다. 본 프로젝트는 그중 클래스 1에 해당하는 경적소리 사운드를 추출하는것을 목적으로 딥러닝 모델을 사용하였다. 딥러닝 모델은 CNN모델을 사용하였다. 지하철 음성인식 기능은 구글 open API 기능을 활용하였다. Google Cloud Speech API는 지원 자연 언어로는 한국어 포함 80여개의 언어를 지원하며, 컴퓨터 언어를 가장 다양하게 지원한다. 이에 따라 활용 분야도 가장 다양하였다. 또한 스마트폰 뿐만 아니라 다양한 IoT 기기에도 API를 활용할 수 있다. Google Cloud Speech API의 성능을 테스트하기 위해 한국어 음성인식이 지원되는 숫자 20단어를 입력하여 Google Speech에서의 숫자 음성 오인식률은 34%이다. 하지만 문장 음성 오인식률은 임의로 만든 문장의 첫 시작 부분인 ‘화요일’을 제대로 인식하지 못한 경우를 제외하면 모든 문장의 단어를 인식했다. 여기서 임의로 만든 문장은 ‘최근 찍은 사진 친구한테 보내줘’, ‘화요일 오후 3시 30분에 알람해줘’이다. 
