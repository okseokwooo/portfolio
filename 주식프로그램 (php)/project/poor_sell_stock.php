<?php
        ...
        $sql = "select * from ai where num>=27 and num<=35 and buy_stock_name!='null' order by rand() limit 1 " ; // 1번 부터 5번까지 주식을 가지고 있는 사람랜덤으로 한명 생성
        ...
        if($random==$random2 && $buy_stock_num>0){
            ...
            mysqli_close($con2);
            header("Refresh:1; url=poor_sell_stock.php");  //페이지 새로고침
        }
        else{
            header("Refresh:1; url=poor_sell_stock.php"); //페이지 새로고침
        }
