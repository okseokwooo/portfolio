<?php
        ...
        $sql = "select * from ai where num>=6 and num<=10 order by rand() limit 1" ; // 1번 부터 5번까지 랜덤으로 한명 생성
        ...
        $pay_money = $stock_money * 400;
        $random = rand(1,8); 
        $random2 = rand(1,8);  //ai가 살 확률을 만들기 위해 만든 변수.
        if($random==$random2 && $money >= $pay_money && $stock_num >= 400 ){
            $money -= $pay_money;
            $stock_money += ($stock_money*400)/100000;  
            $stock_num -= 400;
            $buy_stock_num += 400;
            ...
            mysqli_close($con);
            mysqli_close($con2);
            header("Refresh:0; url=rich_buy_stock.php");  //페이지 새로고침
        }
        else{
            header("Refresh:0; url=rich_buy_stock.php"); //페이지 새로고침
        }
